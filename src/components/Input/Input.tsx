...
export default function Input({
  ...
  classNameEye = 'absolute h-6 w-6 top-3 right-5 cursor-pointer',
  ...rest
}: Props) {
  const [visibleEye, setVisibleEye] = useState(false)
  const registerResult = register && name ? register(name, rules) : null

  const clickVisibleEye = () => {
  ...
  return (
    <div className={'relative ' + className}>
      ...
      {rest.type === 'password' && visibleEye && (
        <svg fill='none' viewBox='0 0 24 24' className={classNameEye} onClick={clickVisibleEye}>
          <path
            stroke='none'
            fill='#000'
            fillOpacity='.54'
      ...
      {rest.type === 'password' && !visibleEye && (
        <svg fill='none' viewBox='0 0 24 14' className={classNameEye} onClick={clickVisibleEye}>
          <path
            stroke='none'
            fill='#000'
            fillOpacity='.54'
